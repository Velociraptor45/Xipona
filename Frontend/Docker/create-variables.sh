...
add_to_json() {
    ...
    if [ -n "$var_value" ]; then
        if [ $needsApostrophes == true ]; then
            echo "  \"$var_name\": \"$var_value\"," >> /usr/share/nginx/html/wwwroot/variables.json
        else
            echo "  \"$var_name\": $var_value," >> /usr/share/nginx/html/wwwroot/variables.json
        fi
    fi
